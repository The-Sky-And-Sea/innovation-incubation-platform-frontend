...
import defaultLanyard from "./lanyard/lanyard.png";
import "./Lanyard.css";
...
function Band({
  ...
  const handlePointerUp = (event: ThreeEvent<PointerEvent>) => {
    ...
    const downDistance = current ? dragStart.current.y - current.y : 0;
    const isQuickClick = performance.now() - pointerDownAt.current < 260 && downDistance < 0.2;
    const isPulled = downDistance > 0.55;
    drag(false);
    if (isPulled || isQuickClick) {
      onToggle?.();
    }
  };
