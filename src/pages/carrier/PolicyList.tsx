...
export default function CarrierPolicyList() {
  ...
  const [form] = Form.useForm<{ materials_json: string }>();
  ...
  const openApply = (policy: Policy) => {
    ...
    form.setFieldsValue({
      materials_json: JSON.stringify(
        policy.requirements?.application_materials || [{ name: "载体服务报告", file_id: 21, necessity: "necessary" }],
        null,
        2,
      ),
    });
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      setApplying(true);
      const materials = values.materials_json
        ? (JSON.parse(values.materials_json) as PolicyMaterial[])
        : [];
      ...
      message.error((err as Error).message || "申报失败，请检查材料 JSON 格式");
    } finally {
      setApplying(false);
    }
  };
  ...
  const columns: ColumnsType<Policy> = useMemo(
    () => [
      ...
      },
      {
        ...
        key: "target_role",
        width: 120,
        render: (value: string) => <Tag color="blue">{value === "both" ? "企业/载体" : "载体"}</Tag>,
      },
      {
        title: "有效期",
        key: "period",
  ...
  return (
    <div>
      ...
      <Card
        extra={
          <Button icon={<ReloadOutlined />} onClick={() => fetchPolicies(pagination.current, pagination.pageSize)} loading={loading}>
            刷新
          </Button>
        }
      >
      ...
      <Modal
        title={
          <Space>
            <SendOutlined />
            <span>申报政策 - {selectedPolicy?.title}</span>
          </Space>
        }
        open={applyOpen}
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item
            name="materials_json"
            label="申报材料 materials"
            tooltip="按接口文档发送 materials 数组。"
            rules={[{ required: true, message: "请填写申报材料" }]}
          >
            <TextArea rows={6} />
          </Form.Item>
        </Form>
